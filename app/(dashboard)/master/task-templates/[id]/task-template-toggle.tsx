...
import { toggleTaskTemplateActive } from "@/actions/template-actions"
import { Switch } from "@/components/ui/switch"
import { Label } from "@/components/ui/label"

export function TaskTemplateToggle({ id, isActive }: { id: string; isActive: boolean }) {
  const [pending, setPending] = useState(false)
  const router = useRouter()

  async function onCheckedChange(checked: boolean) {
    setPending(true)
    const result = await toggleTaskTemplateActive(id, checked)
    setPending(false)
    if (!result.ok) {
